...
void mult_matrix(double* out, double* left, double* right,
    ...
    for (i = 0; i < rows_left; ++i) {
        for (j = 0; j < cols_right; ++j) {
            ...
            #pragma omp parallel for reduction(+:out[i * cols_right + j])
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < sz; ++i) {
        for (int j = 0; j < sz; ++j) {
            int len = snprintf(tmp, sizeof(tmp), "%ld", (long)c[i * sz + j]);
            max_len = max_len > len ? max_len : len;
        }
    }
    char fmt[16];
    if (snprintf(fmt, sizeof(fmt), "%%s%%%dd", max_len) < 0)
        abort();
    for (int i = 0; i < sz; ++i) {
        for (int j = 0; j < sz; ++j)
            printf(fmt, j == 0 ? "" : " ", (long)c[i * sz + j]);
        printf("\n");
    }
